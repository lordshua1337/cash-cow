...
export async function generateBriefPDF(calf: Calf, briefContent: string): Promise<void> {
  ...
  const scoreColor = calf.overallScore >= 70 ? '#059669' : calf.overallScore >= 50 ? '#22C55E' : '#DC2626'

  const html = `
    <div style="font-family: 'Inter', -apple-system, sans-serif; color: #3D2B1F; max-width: 700px; margin: 0 auto;">
      <!-- Cover page -->
      <div style="text-align: center; padding: 60px 40px; border-bottom: 3px solid #22C55E; margin-bottom: 40px;">
      ...
      <div style="margin-bottom: 32px; padding: 16px; border-radius: 8px; background: #FFF8F0; border: 1px solid #F5E6D3;">
        ...
        <div style="display: flex; gap: 24px; text-align: center;">
          <div><div style="font-size: 20px; font-weight: 900; color: #22C55E;">${calf.marketDemandScore}</div><div style="font-size: 10px; color: #8B7355;">Market</div></div>
          ...
          <div><div style="font-size: 20px; font-weight: 900; color: #22C55E;">${calf.revenuePotentialScore}</div><div style="font-size: 10px; color: #8B7355;">Revenue</div></div>
      ...
      <div style="font-size: 13px; line-height: 1.7; color: #3D2B1F;">
        ${briefContent
          ...
          .replace(/- /g, '<span style="color: #22C55E; margin-right: 4px;">-</span>')
        }
      </div>

      <!-- Footer -->
      <div style="margin-top: 40px; padding-top: 16px; border-top: 2px solid #22C55E; text-align: center;">
